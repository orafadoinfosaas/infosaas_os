...
// Capa de carrossel / post estático: headline grande + subheadline de apoio.
const COVER_TYPO: SlideTypo = { headlineSize: 80, subSize: 40, bodySize: 40, order: 'headline-first' }

// Slides de conteúdo (2–9): densidades variadas — rotação dos padrões b.1–b.5.
const CONTENT_PATTERNS: SlideTypo[] = [
  ...
  { headlineSize: 80, subSize: 40, bodySize: 60, order: 'body-first' },     // b.3 (corpo antes da headline)
...
// Tamanhos do slide. null = usa a tipografia da base (ex.: anúncio).
function slideTypo(content: Content, index: number): SlideTypo | null {
  if (content.content_type === 'anuncio') return null
  if (content.content_type !== 'carrossel') return COVER_TYPO // post / estático / stories
...
// Mede a altura real do texto com o MESMO wrap por palavra do Konva, usando
// canvas measureText (métricas reais da fonte). Garante que os blocos nunca se
// sobreponham, independente de tamanho/fonte escolhidos.
...
function countLines(text: string, fontSize: number, weight: number, family: string, letterSpacing: number, maxWidth: number): number {
  ...
  const style = weight >= 600 ? 'bold' : 'normal' // Konva usa bold/normal
...
function assetUrl(slug: string | undefined, ref: string): string {
  if (/^https?:\/\//.test(ref)) return ref
  return `/api/assets/${slug ?? 'preview'}/${ref}`
}
...
export function composeFrame(
  ...
  // anúncio: headline ativo escolhido no preview
  if (content.content_type === 'anuncio') {
  ...
  const onMedia = isCover // texto só fica "sobre a mídia" no modo cover
  ...
  // Máscara só no cover (scrim de legibilidade). 'media' é montado após o box do elemento.
  ...
  // ── Branding (calculado antes do texto p/ reservar área segura) ──
  ...
  const logo: RichLogo | null = branding.logo.show
    ? { src: `/brand/logo-${LOGO_FILE[branding.logo.variant]}.svg`, position: branding.logo.position, height: LOGO_H }
    : null
  ...
  // Área segura: o texto não invade as faixas onde há logo/numeração/handle.
  ...
  // ── Imagem-elemento ──
  // topo/base = banda na borda; meio = bloco no fluxo, logo após a headline.
  ...
  type Block = {
    ...
    mt: number; mb: number; px: number // margins/padding por bloco
  }
  const blocks: Block[] = []
  ...
  // Tamanhos por slide (spec): capa/post grandes; conteúdo varia por padrão.
  ...
  const gap = 18
  // Medição respeita o paddingX de cada bloco (texto fica mais estreito).
  const heights = blocks.map((b) => blockHeight(b.text, b.size, b.weight, b.font, b.ls, b.lh, maxWidth - 2 * b.px))

  // Sequência de itens: no modo 'middle' a imagem entra após a headline.
  ...
  // Total já inclui as margens verticais por bloco — preserva o alinhamento correto.
  ...
  if (cursorY < areaTop) cursorY = areaTop // nunca começa sob a faixa de marca
  ...
  items.forEach((it, k) => {
    ...
    if (it.kind === 'image') {
      ...
      cursorY += b.mt // margem superior do bloco
      const x = PAD + b.px
      ...
      cursorY += h + b.mb + gap // texto + margem inferior + gap entre itens
    }
  })

  const media: RichMedia | null = isCover
    ? { src: assetUrl(slug, design.media.ref), box: null, fit: design.media.fit, radius: 0 }
    : isElement
      ? { src: assetUrl(slug, design.media.ref), box: elementBox, fit: design.media.fit, radius: design.media.radius ?? 0 }
      : null
