import { z } from 'zod'
import { makeFormat } from './schemas/format.schema.js'
import type { Content, FunnelPhase, TemplateId } from './schemas/content.schema.js'
import type { ContentType } from './types.js'

// ─── Schemas "criativos" ──────────────────────────────────────────────────────
// Só o que a LLM deve produzir (texto + estrutura). Os campos controlados pelo
// app/MCP (id, datas, content_type, format, imagens, cta, layout…) são montados no
// servidor — elimina tipo errado e campos faltando (o host não adivinha o Content).
